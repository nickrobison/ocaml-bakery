...
module Make(C: Sig.Caveat)(M: Sig.Macaroon with type c = C.t)(V: Sig.Verifier with type m = M.t) = struct
  ...
  Alcotest.(check macaroonTest) "Should deserialize with no caveats" m (M.deserialize "MDAxY2xvY2F0aW9uIGh0dHA6Ly9teWJhbmsvCjAwMjZpZGVudGlmaWVyIHdlIHVzZWQgb3VyIHNlY3JldCBrZXkKMDAyZnNpZ25hdHVyZSDj2eApCFJsTAA5rhURQRXZf91ovyujebNCqvD2F9BVLwo");
  ...
  Alcotest.(check macaroonTest) "Should deserialize with caveats" m2 (M.deserialize "MDAxY2xvY2F0aW9uIGh0dHA6Ly9teWJhbmsvCjAwMjZpZGVudGlmaWVyIHdlIHVzZWQgb3VyIHNlY3JldCBrZXkKMDAxZGNpZCBhY2NvdW50ID0gMzczNTkyODU1OQowMDJmc2lnbmF0dXJlIB7-R2PykNvODB0IR3Nn4R9O7kVqZJM89mLXl3LbuCEoCg");
  ...
  Alcotest.(check macaroonTest) "Should round trip ok" m3 (M.deserialize (M.serialize m3 M.V1))
...
let verifier_tests () =
  ...
  (  match (V.verify v m2 key) with
     ...
     | Error _ -> ());
  let v = V.add_first_party_caveat v "account = 3735928559" in
  match (V.verify v m2 key) with
  | Ok _ -> ()
